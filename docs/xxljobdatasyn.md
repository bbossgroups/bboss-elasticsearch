...
一个基于spring组件定义的数据同步作业组件SyncDataXXLJob（database到elasticsearch数据同步）：
 ...
 */
@Component
public class DatatranXxlJob {
    private static Logger logger = LoggerFactory.getLogger(DatatranXxlJob.class);
    ...
    public void demoJobHandler2() throws Exception {
       try {
          ...
          externalScheduler.execute(  param);
           XxlJobHelper.handleSuccess("作业执行ok！");
       }
       finally {
          lock.unlock();
       }
...
组件说明：通过XxlJob注解将方法demoJobHandler2标记为xxl-job作业执行方法

1. XxlJob注解value属性指定了作业名称，init属性指定xxl-job作业初始化方法，destroy属性指定xxl-job作业销毁方法
2. 在init方法里面初始化数据同步作业组件ExternalScheduler
3. 在demoJobHandler2方法中调度也运行数据同步作业功能
4. 在destroy方法中注销作业组件ExternalScheduler，释放资源
