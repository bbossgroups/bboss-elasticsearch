...
- [**通用分布式作业调度工作流**](https://esdoc.bbossgroups.com/#/jobworkflow)，提供通用流程编排模型，支持有向循环图；可以将各种需要按照先后顺序执行的任务编排成工作流，进行统一调度执行，譬如数据采集作业任务、流批处理作业任务、业务办理任务、充值缴费任务以及大模型推理任务等，按照顺序编排成工作流程；默认提供了数据交换和流批处理作业节点、通用函数节点以及复合类型节点（串/并行执行），可以按需自定义扩展新的流程节点、远程服务执行节点；可以为流程节点设置条件触发器，控制流程节点是否执行；通过流程上下文和节点上下文在节点间传递和共享参数；通过设置流程执行和节点执行监听器，维护流程和节点执行参数，采集和获取流程、节点执行监控指标以及执行异常信息。
...
6. 智能体框架改进：并行子智能体增加动态[构建并行分支智能体节点功能](https://esdoc.bbossgroups.com/#/bboss-ai?id=_1434-%e5%8a%a8%e6%80%81%e6%9e%84%e5%bb%ba%e5%b9%b6%e8%a1%8c%e5%ad%90%e6%99%ba%e8%83%bd%e4%bd%93)
7. 智能体框架改进：串行子智能体流程可以动态构建串行分支子智能体流程节点
8. 智能体框架改进：工作流改进，并行工作流增加并行分支分组id和父分支分组id，用以支撑智能体工作流并行子智能体向客户端并行输出子智能体消息
9. 智能体框架改进：[完善人工介入任务，增加人工任务参数传递以及用户提交信息处理回调机制](https://gitee.com/bboss/bboss-elastic-tran/commit/8566008ff85bc5f06334674e2d9e265ee00601ee)
10. 智能体框架改进：会话删除，增加人工任务删除功能，修改Clickhouse会话表字段定义，String类型字段允许空值字段
11. 智能体框架改进：增加mcp工具调用失败重试次数，使用方法：https://esdoc.bbossgroups.com/#/bboss-ai?id=_9441-%e5%a4%b1%e8%b4%a5%e9%87%8d%e8%af%95
12. 智能体框架改进：[处理mcp client与spring ai mcp server兼容性问题，完善mcp服务调用异常机制，将mcp服务端返回的异常错误信息反馈给大模型](https://gitee.com/bboss/bboss-ai/commit/34295ae90863907b25ab6eeb57e7d1f4c2d5cb91)
13. 智能体框架改进：将设置到agentMessage的提示词默认设置为input.query变量，但是可以被工作流过程中设置的同名变量覆盖
14. 智能体框架改进：扩展工作流提示词管理功能：变量体系中type=url时，增加指定httpproxy服务属性；增加type=service类型，可以从本地服务组件中获取提示词内容
...
5. 完善持久层事务管理接口：处理Clickhouse官方驱动connection对象不支持事务commit、rollback、setAutocommit管理接口问题
6. 完善文件采集插件文件采集状态管理机制：作业启动时只加载未完成状态的文件记录和丢失状态的文件记录信息，优化历史记录迁移机制：
   在扫描新文件时，才加载已完成或者以及迁移到历史状态表的文件采集记录信息，如果存在则说明已经采集过，忽略相关文件的采集处理；
   在扫描已过期的完成文件记录到历史表时，需要将completeTasks缓存的已过期记录迁移到lazeCompleteTasks，并将其标记为历史状态；
   将lazeCompleteTasks中已过期的非历史状态的采集记录标记为历史状态，并将其迁移到历史记录表中
7. 完善文件采集插件文件采集状态管理机制：优化历史记录迁移机制，先判断内存中驻留的完成
8. 完善文件采集插件文件采集状态表升级机制：检测字段不存在时，不要召回数据库记录，提升启动性能
9. 工作流文件下载节点改进：完善已下载文件状态管理机制
10. 内存管理优化：去除BaseStatusManager中定义的jvm退出钩子,存在内存泄漏隐患，去掉其他模块相关jvm退出钩子 
11. 优化IP地址解析组件：改进destroy和closedb方法
12. 工作流节点监听器异常处理机制优化，打印异常堆栈信息到日志文件中
13. 数据交换数据库输出插件改进：sql中为未设置参数变量时，报错信息中给出sql配置名称和原始sql信息
14. IP解析组件改进：升级IP2RegionV2版本源码
15. 升级持久层依赖的commons-dbcp2和commons-pool2版本：
...
```json
{
    "id": #[id,type=int,default=10,description=唯一标识]
}
```
20. bboss-ai客户端改进：将AI客户端从bboss-http5剥离，新增bboss-ai模块，依赖bboss-http5模块与模型maas平台交互
...
50. 数据交换问题修复：修复ip地址解析时省份信息处理不正确问题    
51. 智能体框架改进：mcp和工具函数增加[工具搜索过滤](https://esdoc.bbossgroups.com/#/bboss-ai-toolsearcher)功能，增加关键词路由智能体功能
...
3. 优化数据库管理工具
4. [mysqlbinlog同步改进](https://esdoc.bbossgroups.com/#/datatran-plugins)：自定义BinaryLogClientExt，打印异常情况日志
5. mysqlbinlog同步改进：修复mysqlbinlog多表同步不起作用问题
6. 去除commons-lang依赖,升级为commons-lang3
...
29. [ftp文件采集插件](https://esdoc.bbossgroups.com/#/filelog-guide?id=_7ftp%e9%87%87%e9%9b%86%e9%85%8d%e7%bd%ae)改进：修改框架ftp协议模式，增加ftpdebug模式，支持调试输出ftp指令到日志文件，改进ftp协议下的listFile机制
...
# v7.3.6 功能改进-20250309
1. Elasticsearch客户端改进：规范化mgetDocuments方法命名规范，避免潜在的方法执行冲突，并修改[mget使用文档](https://esdoc.bbossgroups.com/#/Elasticsearch-Mget-GetDocSource-partupdate)
...
**因此升级到7.5.6时需要对采集作业的FileFilter接口方法accept进行相应调整**
...
3. 运行容器工具改进：完善[运行容器工具](https://my.oschina.net/bboss/blog/469411)，增加启动bootrap类，负责运行、停止、重启mainclass，并将mainclass运行、停止、重启过程中的日志、异常输出到log日志文件
  ...
  **gradle坐标**
  ...
  ```java
  group: 'com.bbossgroups', name: 'bboss-bootstrap-rt', version: "6.3.5",transitive: true 
  ```
...
gradle坐标：
```xml
[group: 'com.bbossgroups', name: 'bboss-spring-boot-starter', version: " 6.5.3", transitive: true]
```
使用案例：
