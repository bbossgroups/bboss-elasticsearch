...
借助bboss可以非常方便地将ftp和本地文件目录下的文件数据导入到不同的数据库表中，亦可以将数据导入到不同的elasticsearch索引中，以及其他输出目的地。
...
gradle坐标
```xml
api 'com.bbossgroups.plugins:bboss-datatran-fileftp:7.5.5'
```
...
| FtpConfig.debugMode | 可选，Boolean类型，ftpdebug模式，支持调试输出ftp指令到日志文件中（必须info级别日志） |
