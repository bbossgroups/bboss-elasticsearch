...
​		bboss jobflow **通用分布式作业调度工作流**，提供通用轻量级、高性能流程编排模型，可将各种各样、不同类型的任务编排成工作流，进行统一调度执行，譬如数据采集作业任务、流批处理作业任务、业务办理任务、充值缴费任务以及大模型推理任务等按顺序编排成工作流；通过有向循环图，实现多智能体协同、Re-Act智能体任务自主规划、迭代执行、结果评估和优化。
...
通过以下执行上下文来在流程节点之间传递和共享参数

1） 流程上下文：在流程各节点之间传递和共享数据、流程参数和状态

2）节点上下文：节点内部子节点之间传递和共享数据、节点参数和状态
...
可以为各种类型的流程节点添加条件触发器，控制流程节点是否执行，实现条件分支功能：
