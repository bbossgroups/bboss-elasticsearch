...
插件配置 [FeishuTableInputConfig](https://gitee.com/bboss/bboss-elastic-tran/blob/master/bboss-datatran-core/src/main/java/org/frameworkset/tran/plugin/feishu/input/FeishuTableInputConfig.java)

通过FeishuTableInputConfig设置飞书数据源、飞书多维表格appToken、tableid、授权应用appId和appSecret等参数，配置字段级别值转换器、全局字段值转换器；飞书查询参数请求报文（查询字段清单、条件过滤器、排序字段、多维表格viewid等）
...
/**
		 * 输入插件配置
		 */
		FeishuTableInputConfig feishuTableInputConfig = new FeishuTableInputConfig();
		if(!outSource) {
			//飞书数据源定义
			feishuTableInputConfig.addHttpConfig("http.poolNames", "feishu");
			
			feishuTableInputConfig.addHttpConfig("feishu.http.hosts", "https://open.feishu.cn");
			feishuTableInputConfig.addHttpConfig("feishu.http.maxTotal", 100);
			feishuTableInputConfig.addHttpConfig("feishu.http.defaultMaxPerRoute", 100);
			
			
		}
		else{//引用外部定义的数据源
			feishuTableInputConfig.setFeishuDataSource("feishu");
		}
		feishuTableInputConfig.setFeishuTableId("tblDeYxb");
		feishuTableInputConfig.setFeishuTableAppToken("F1JHMOjaZ");
		feishuTableInputConfig.setFeishuAppId("cli_a90febbc");
		feishuTableInputConfig.setFeishAppSecret("RNhMgNhysT5tmGeKZIsB");
		feishuTableInputConfig.registFieldValueConvertor("指标签约时间", new FieldValueConvertor() {
			@Override
			public Object handleItem(Map<String, Object> fields, String field, Object value) {
				Number numbervalue = (Number) value;
				Date timestamp = new Date(numbervalue.longValue());
		...
		String requestBody = """
							...
							""";
		feishuTableInputConfig.setRequestBody(requestBody);
		importBuilder.setInputConfig(feishuTableInputConfig);
		
		importBuilder.setFetchSize(100);//分页查询记录数，最多500条
```
...
```java
CustomOutputConfig customOutputConfig = new SimpleCustomOutputConfig(){
			
			@Override
			protected void handleData(CustomOutPutContext customOutPutContext) {
				List<CommonRecord> datas = customOutPutContext.getDatas();
				for(CommonRecord record:datas) {
					Map<String, Object> data = record.getDatas();
					logger.info(JsonUtil.object2json(data));
					logger.info("recordId:{}",(String)record.getMetaValue("recordId"));
				}
			}
		};
...
```java
FeishuTableOutputConfig feishuTableOutputConfig = new FeishuTableOutputConfig();
		
		if(!outSource) {
			//飞书数据源定义
			feishuTableOutputConfig.addHttpConfig("http.poolNames", "feishu");
			
			feishuTableOutputConfig.addHttpConfig("feishu.http.hosts", "https://open.feishu.cn");
			feishuTableOutputConfig.addHttpConfig("feishu.http.maxTotal", 100);
			feishuTableOutputConfig.addHttpConfig("feishu.http.defaultMaxPerRoute", 100);
			
			
		}
		else{
			feishuTableOutputConfig.setFeishuDataSource("feishu");
		}
		//飞书表格信息定义
		feishuTableOutputConfig.setFeishuTableId("tblDeEAgy");
		feishuTableOutputConfig.setFeishuTableAppToken("F1JHb8MOjaZAYb");
		feishuTableOutputConfig.setFeishuAppId("cli_a90fe");
		feishuTableOutputConfig.setFeishAppSecret("RNhMgNhysT");
		
		
		//飞书表格映射定义
		feishuTableOutputConfig.addCellMapping("名称","NAME");
		feishuTableOutputConfig.addCellMapping("类型","TYPE");
		feishuTableOutputConfig.addCellMapping("状态","STATE");
		 
		
		importBuilder.setOutputConfig(feishuTableOutputConfig);	
```
