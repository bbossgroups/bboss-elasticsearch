			...
			try {
				...
				if(!this.isFromspringboot()){
					if(logger.isDebugEnabled()){
						Exception exception = new Exception("Debug Elasticsearch Datasource["+this.getElasticSearchName()+"] start trace:if use spring boot and do not load spring boot config right,please get the reason from question-answer document:https://esdoc.bbossgroups.com/#/question-answer ,if not or do not care,ignore this debug message. ");
